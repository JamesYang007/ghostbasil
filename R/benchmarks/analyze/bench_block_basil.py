...
def analyze(baseline_data_path,
        ...
        plt.plot(L, baseline, ls='-', label='baseline')
        plt.plot(L, new_version, ls='--', label='new_version')
        plt.title(f"Block Matrix Basil (p={p})")
        plt.tight_layout()
        plt.legend()
        plt.savefig(os.path.join(fig_path, 'block_basil.png'))
